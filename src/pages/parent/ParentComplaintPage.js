...
function ParentComplaintPage() {
  ...
  /*
   * Transformation des données venant du backend
   * vers le format utilisé actuellement par l'interface.
   */
  const formatComplaint = useCallback((complaint) => {
    return {
      id: complaint.id,
  ...
  /*
   * Récupération des réclamations du parent
   * depuis Spring Boot / MySQL.
   */
  ...
  }, [currentUser?.id, formatComplaint]);
  ...
  /*
   * Envoi de la réclamation vers Spring Boot.
   */
  const handleSubmit = async (event) => {
    ...
    if (!currentUser?.id) {
      console.error('Impossible d’envoyer la réclamation : utilisateur non connecté.');
      return;
    }
    ...
    try {
      ...
      /*
       * Conversion pour l'affichage frontend.
       */
      const formattedComplaint = formatComplaint(createdComplaint);

      /*
       * On ajoute directement la nouvelle réclamation
       * dans l'historique affiché.
       */
      ...
      /*
       * Notifications email.
       * Elles restent indépendantes de l'enregistrement
       * en base de données.
       */
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <p className="mt-4 text-sm text-slate-600 dark:text-slate-300">
          {t('parentSpace.complaint.description')}
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            ...
            <input
              ...
              placeholder={t(
                'parentSpace.complaint.fields.subjectPlaceholder'
              )}
          ...
          {loading && myComplaints.length === 0 ? (
            ...
            myComplaints.map((complaint) => (
              ...
              >
                ...
                {complaint.note && (
                  <p className="mt-3 rounded-2xl bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
