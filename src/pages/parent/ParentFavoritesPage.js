  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">{t('parentSpace.favorites.tag')}</p>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{t('parentSpace.favorites.title')}</h2>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {favoriteSitters.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            <Trans i18nKey="parentSpace.favorites.emptyText">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Link to={`/espace-parent/babysitter/${sitter.id}`} className="rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-4 py-2 text-sm font-semibold text-white">{t('parentSpace.favorites.viewProfile')}</Link>
                  <button type="button" onClick={() => handleRemoveFavorite(sitter.id)} className="rounded-full border border-red-200 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 dark:border-red-600/40 dark:hover:bg-red-900/20">{t('parentSpace.favorites.remove')}</button>
                </div>
              </div>
            </div>
          </div>
