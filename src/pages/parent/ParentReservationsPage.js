...
function ParentReservationsPage() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">{t('parentSpace.reservations.newTag')}</p>
        <h3 className="mt-3 text-xl font-extrabold text-slate-900 dark:text-slate-100">{t('parentSpace.reservations.newTitle')}</h3>
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.sitter')}
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.date')}
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.hour')}
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.duration')}
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.address')}
          ...
          <div className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            {t('parentSpace.reservations.fields.payment')}
            <div className="mt-2 grid grid-cols-2 gap-3">
              {[{ value: 'sur_place', label: t('parentSpace.reservations.paymentOnSite') }, { value: 'carte', label: t('parentSpace.reservations.paymentCard') }].map((option) => (
          ...
          <button type="submit" className="w-full rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-5 py-3 text-sm font-semibold text-white">{t('parentSpace.reservations.save')}</button>
        </form>
        <div className="mt-6 rounded-3xl bg-slate-50 p-4 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          <p className="font-semibold">{t('parentSpace.reservations.pendingCount', { count: pendingReservations.length })}</p>
          <p className="mt-2">{t('parentSpace.reservations.pendingNote')}</p>
        </div>
      </div>
    </div>
  );
