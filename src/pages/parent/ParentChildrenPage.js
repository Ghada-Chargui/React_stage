  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">Profils enfants</p>
        <h2 className="mt-3 text-2xl font-extrabold text-slate-900 dark:text-slate-100">Vos enfants</h2>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Ces informations aident les babysitters à mieux préparer chaque garde.</p>

        <div className="mt-6 space-y-4">
          {children.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-slate-400">Aucun profil enfant ajouté pour le moment.</p>
          ) : children.map((child) => (
            <div key={child.id} className="rounded-3xl border border-slate-200 p-4 dark:border-slate-700">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-extrabold text-slate-900 dark:text-slate-100">{child.name} • {child.age} ans</p>
                    {child.allergies && <p className="mt-1 text-sm text-red-600 dark:text-red-400">Allergies : {child.allergies}</p>}
                    {child.notes && <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{child.notes}</p>}
                  </div>
                </div>
                <div className="flex shrink-0 gap-2">
                  <button type="button" onClick={() => handleEdit(child)} className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Modifier</button>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">{editingId ? 'Modifier' : 'Ajouter'}</p>
        <h3 className="mt-3 text-xl font-extrabold text-slate-900 dark:text-slate-100">{editingId ? 'Modifier un profil enfant' : 'Créer un profil enfant'}</h3>
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            Allergies (optionnel)
            <input value={form.allergies} onChange={(event) => setForm({ ...form, allergies: event.target.value })} placeholder="Ex. arachides" className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800" />
          </label>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
            Notes pour la babysitter
          ...
          <div className="flex gap-3">
            <button type="submit" className="rounded-full bg-gradient-to-r from-orange-600 to-amber-600 px-6 py-3 text-sm font-semibold text-white">{editingId ? 'Enregistrer' : 'Ajouter l’enfant'}</button>
            {editingId && <button type="button" onClick={resetForm} className="rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-300">Annuler</button>}
          </div>
        </form>
      </div>
    </div>
