...
function BabysitterReviewsPage() {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">{t('babysitterSpace.reviews.tag')}</p>
      <div className="mt-4 flex items-center justify-between rounded-3xl bg-amber-50 p-5 dark:bg-amber-900/20">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{t('babysitterSpace.reviews.averageTitle')}</h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{t('babysitterSpace.reviews.averageSubtitle')}</p>
        </div>
        <div className="text-3xl font-extrabold text-amber-700">{average}{average !== '—' ? '/5' : ''}</div>
      </div>
      <div className="mt-6 space-y-4">
        {reviews.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('babysitterSpace.reviews.empty')}</p>
