  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-extrabold uppercase tracking-[0.32em] text-orange-600">{t('babysitterSpace.history.tag')}</p>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">{t('babysitterSpace.history.title')}</h2>
        </div>
      </div>

      <div className="mt-6 space-y-4">
        {pastRequests.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('babysitterSpace.history.empty')}</p>
          ...
          <div key={request.id} className="rounded-3xl border border-slate-200 p-4 dark:border-slate-700">
            ...
            {request.review && (
              <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                {t('babysitterSpace.history.reviewReceived')} : ★ {request.review.stars} — {request.review.comment}
              </div>
            )}
          </div>
        ))}
