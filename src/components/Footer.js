    ...
    <footer className="border-t border-slate-200 bg-gradient-to-br from-white to-amber-50 py-16 dark:border-slate-700 dark:from-slate-900 dark:to-slate-950">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 sm:px-6 lg:px-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-5">
          ...
          <div>
            <p className="text-sm font-extrabold uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{t('footer.title')}</p>
            <p className="mt-3 max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-300">{t('footer.description')}</p>
          </div>
        </div>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-8">
          <Link to="/inscription" className="text-slate-700 font-semibold transition-all duration-300 hover:text-orange-700 hover:underline dark:text-slate-200 dark:hover:text-orange-400">{t('footer.signup')}</Link>
        </div>
      </div>
